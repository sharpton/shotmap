...
my $cmd1 = "perl ${master}/scripts/build_shotmap_searchdb.pl -r $t_fams -d $t_fam_db --force-searchdb --verbose --nprocs $nprocs" ;
print $cmd1 . "\n";
`$cmd1`;
...
`cp $sample $output`;
my $cmd2 = "perl ${master}/scripts/shotmap.pl -i ${output}/${stem} -d $t_fam_db --ags-method=none --clobber --verbose  --nprocs $nprocs";
print $cmd2 . "\n";
`$cmd2`;
#test shotmap on a single, specified sample, distinct ffdb
my $out3 = File::Spec->catdir( $output, $stem . "_sm_ffdb");
my $cmd3 = "perl ${master}/scripts/shotmap.pl -i $sample -d $t_fam_db -o $out3 --ags-method=none --clobber --verbose --nprocs $nprocs";
print $cmd3 . "\n";
`$cmd3`;
...
my $cmd5 = "perl ${master}/scripts/shotmap.pl -i $samp_dir -d $t_fam_db -o $out5 --ags-method=none --clobber --verbose --nprocs $nprocs";
print $cmd5 . "\n";
`$cmd5`;
##################
# ADD TESTS HERE
