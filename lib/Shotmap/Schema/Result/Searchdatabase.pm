use utf8;
package Shotmap::Schema::Result::Searchdatabase;

# Created by DBIx::Class::Schema::Loader
# DO NOT MODIFY THE FIRST PART OF THIS FILE

=head1 NAME

Shotmap::Schema::Result::Searchdatabase

=cut

use strict;
use warnings;

use base 'DBIx::Class::Core';

=head1 TABLE: C<searchdatabases>

=cut

__PACKAGE__->table("searchdatabases");

=head1 ACCESSORS

=head2 searchdb_id

  data_type: 'integer'
  is_auto_increment: 1
  is_nullable: 0

=head2 db_type

  data_type: 'varchar'
  is_nullable: 0
  size: 256

=head2 db_name

  data_type: 'varchar'
  is_nullable: 0
  size: 256

=cut

__PACKAGE__->add_columns(
  "searchdb_id",
  { data_type => "integer", is_auto_increment => 1, is_nullable => 0 },
  "db_type",
  { data_type => "varchar", is_nullable => 0, size => 256 },
  "db_name",
  { data_type => "varchar", is_nullable => 0, size => 256 },
);

=head1 PRIMARY KEY

=over 4

=item * L</searchdb_id>

=back

=cut

__PACKAGE__->set_primary_key("searchdb_id");

=head1 UNIQUE CONSTRAINTS

=head2 C<name_type>

=over 4

=item * L</db_name>

=item * L</db_type>

=back

=cut

__PACKAGE__->add_unique_constraint("name_type", ["db_name", "db_type"]);


# Created by DBIx::Class::Schema::Loader v0.07035 @ 2013-09-04 11:41:56
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:L8qjIM6wmImApE/m/KQYpg


# You can replace this text with custom code or comments, and it will be preserved on regeneration
1;
